function Qnew=pos_def02(U,Q,dx,dt,ord,BC)
...
small=1e-30;
jp=length(Q);
f_plus=zeros(1,jp+ord1+2+ord1+3);
f_minus=zeros(1,jp+ord1+2+ord1+3);
fj1=zeros(1,jp+ord1+2+ord1+3);
fj2=zeros(1,jp+ord1+2+ord1+3);
Qold=zeros(1,jp+ord1+2+ord1+3);
u1=zeros(1,jp+ord1+2+ord1+3);
        ...
        if(monotonic ~= 1)
            for k=0:ord
                ...
                f_plus(j)=f_plus(j)+dx./dt.*a_coeff01(k+1)./ ...
                    ((1+k).*2.^(1+k)).*(1-(1-2.*cp_j).^(1+k));
            end
        else
            for k=0:ord
                % step 1: calculate the monotone fluxes
                 f_plus(j)=f_plus(j)+dx./dt.*a_coeff01(k+1)./ ...
                    ((1+k).*2.^(1+k)).*(1-(1-2.*cp_jm1).^(1+k));
            end            
        end
    end
    if(monotonic==1)
        ...
        if(u1(j) >0 & u1(j-1) >0) 
           dummy1=dx./dt.*(Qold(j)-max(Qold(j-1),Qold(j)))+f_plus(j-1); 
           dummy2=dx./dt.*(Qold(j)-min(Qold(j-1),Qold(j)))+f_plus(j-1); 
           f_plus(j)=min(dummy2,f_plus(j));
           f_plus(j)=max(dummy1,f_plus(j));
           f_plus(j)=min(dummy2,f_plus(j));       
        end
        % step 3: add the deformation terms, Eq. 21, Bott (1992)
        f_plus(j)=f_plus(j)+dp_jp05;
    end
        ...
        if(monotonic ~=1)
            for k=0:ord
                f_minus(j-1)=f_minus(j-1)+dx./dt.*a_coeff01(k+1)./ ...
                    ((1+k).*2.^(1+k)).*((-1).^k).*(1-(1-2.*cm_jm1).^(1+k));
            end
        else
            for k=0:ord
                 f_minus(j-1)=f_minus(j-1)+dx./dt.*a_coeff01(k+1)./ ...
                    ((1+k).*2.^(1+k)).*((-1).^k).*(1-(1-2.*cm_j).^(1+k));
            end           
        end
    end
    if(monotonic==1)
        % step 2: apply the monotone flux limiter
        if(u1(j) <0 & u1(j-1) < 0) 
           dummy1=dx./dt.*(Qold(j)-max(Qold(j+1),Qold(j)))+f_minus(j); 
           dummy2=dx./dt.*(Qold(j)-min(Qold(j+1),Qold(j)))+f_minus(j); 
           f_minus(j-1)=min(dummy2,f_minus(j-1));
           f_minus(j-1)=max(dummy1,f_minus(j-1));
           f_minus(j-1)=min(dummy2,f_minus(j-1));       
        end
        % step 3: add the deformation terms, Eq 21, Bott (1992)
        f_minus(j-1)=f_minus(j-1)+dm_jm05;
    end
    % step 4: apply the positive definite flux limiter (as in bott, 1989)
