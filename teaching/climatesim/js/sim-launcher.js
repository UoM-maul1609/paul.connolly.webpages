...
$(function() {
	...
	function resize() {
		...
		// Keep the original desktop arrangement, but size primarily from the
		// available width. The page can scroll vertically, so a short browser
		// window should not collapse the simulation to an unusably small square.
		if (windowWidth > Config.breakpoint) {
			canvasSize = Math.max(360, Math.min(700, windowWidth - 370));
			...
			allSliders.each(function() {
				...
				slider.css({
					"height": Math.max(230, canvasSize - 30),
					"width": ""
				});
			});
			...
			// Retain the original horizontal-slider layout on tablets and phones.
			// On very small screens put the thermometer above the scene so the
			// canvas is not squeezed into a narrow strip.
			var compact = windowWidth <= 560;
			canvasSize = compact
				? Math.max(260, Math.min(520, windowWidth - 24))
				: Math.max(260, Math.min(650, windowWidth - 170));

			allSliders.slider("option", "orientation", "horizontal").css({
				"height": "",
				"width": compact ? Math.max(120, canvasSize - 155) : Math.max(140, canvasSize - 170)
			});
			...
			container.css("margin-left", compact ? 0 : 130);
			climateSim.css("width", compact ? "calc(100% - 20px)" : canvasSize + 140);
		}
		...
		thermometer.css({
			"height": Math.max(140, canvasSize - 120)
		});

		// Let the commentary grow naturally. Fixed calculated heights could
		// clip longer explanations or create large blank areas.
