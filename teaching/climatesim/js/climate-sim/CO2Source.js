...
CO2Source.prototype.puffCO2 = function() {
	...
	var puffing = function() {
		if (!_this.destroyed) { _this.animateCO2Puff(); }
	};
	this._timers.push(setTimeout(puffing, this.CO2frequency));
	
	var doPuffOfCO2 = function() {
		if (!_this.destroyed) { _this.puffCO2(); }
	};

	// Loop the animation. Keep the timer id so it can be cancelled if
	// the building is removed by the CO2 slider.
	this._timers.push(setTimeout( doPuffOfCO2, this.CO2frequency + (Math.round(500*Math.random()) + 250 ) ));
}
